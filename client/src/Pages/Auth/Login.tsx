...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(err);
      toast.error("Login failed! Please try again."); // Display failure toast
    }
  };
  ...
  return (
    <div className={`flex min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center">
        <Card className={`w-full max-w-md mx-8 shadow-lg border-0 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <CardHeader className="space-y-1 text-center pb-8">
            <h2 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Sign in</h2>
